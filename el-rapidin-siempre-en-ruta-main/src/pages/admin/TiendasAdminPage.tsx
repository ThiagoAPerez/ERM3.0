...
const TiendasAdminPage = () => {
  ...
  const handleSave = async () => {
    await api.post("/admin/businesses", {
      ...
      category: "STORE", // 🔥 CLAVE
      address: formData.direccion,
      municipality: formData.municipio,
      phone: formData.telefono,
      email: `${Date.now()}@store.local`, // temporal si no pides email
      temporaryPassword: "123456", // temporal
      description: formData.descripcion,
      logoUrl: formData.logo || null,
      coverUrl: formData.imagenFondo || null,
    });

    const res = await api.get<BackendStore[]>("/admin/businesses");

    setTiendas(
      res.data
        .filter((b: any) => b.category === "STORE")
        .map((b: any) => ({
          id: String(b.id),
          nombre: b.name,
          categoria: "tienda",
